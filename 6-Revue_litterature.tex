...
\section{Reptation des polymères thermoplastique}
...
Pendant le procédé de soudage, lors de la mise en contact des faces à joindre, les chaînes de polymère mobiles diffusent entre les pièces et nous observons progressivement une homogénéisation de la soudure et la disparition de la frontière entre les pièces (Fig. \ref{fig:polymer_diffusion}). 
...
Les équations développés pour l'atteinte de cet état prennent en compte la rugosité de la surface à l'aide de paramètres géométriques, la pression de contact et la viscosité de la matrice polymérique. 
Les premiers modèles simplifiaient la surface en une série de rectangles idéalisés d'aire constante mais dont la hauteur et la largeur varie en fonction du temps \cite{Lee1987}. 
...
Les modèles basés sur des rectangles idéalisés sont incapables de prédire le comportement depuis des mesures directes de la rugosité des interfaces et nécessitent des paramètres d'ajustement obtenus suite à des essais en laboratoire \cite{Yang2001}. 
Une autre génération de modèle tente plutôt de représenter la surface comme une série de rectangles de tailles diverses parfois semblables à un fractal dont les propriétés peuvent être reliées à des mesures de rugosité des surfaces \cite{Yang2001,Yang2002}. 
Dans ces modèles, le polymère est écrasé jusqu'à ce que tous les rectangles d'aire constante forment une frontière unique. 
À partir de données de rugosité, l'auteur a pu établir un modèle montrant une évolution rapide du critère de contact intime suivi d'un ralentissement à l'approche du plein contact \cite{Yang2001}. 
Dans des conditions isothermes à \SI[locale=FR]{350}{\celsius} avec des pressions de 0,67 et \SI[locale=FR]{1,63}{\mega\pascal}, le contact intime est totalement atteint avec des temps respectivement de \SI[locale=FR]{100}{\second} et de \SI[locale=FR]{40}{\second} pour des laminés en PEEK. 
...
La reconfiguration des chaînes dans le polymère se produit par un phénomène de reptation et de mouvement brownien a été conceptualisé, pour les polymères fondus, assez tôt \cite{DeGennes1971,Edwards1978b,Klein1978,Daoud1979}. 
...
Cette dernière entraîne également la formation d'enchevêtrement de chaînes au travers de l'interface \cite{Wool1983}
...
Tout dépendamment de la nature du polymère, son poids moléculaire, la température et la pression, le processus de cicatrisation peut se produire en quelques minutes ou peut nécessiter plusieurs heures voir même des jours \cite{Prager1981a}. 
...
Pour les polymères amorphes, on peut citer les travaux de Wool durant les années 1980 \cite{Wool1983,Wool1989}. 
...
L'impact de la température sur le temps de reptation peut etre évalué à l'aide d'une équation d'Arrhenius où les paramètres $A_r$ et $B_r$  doivent être évalués expérimentalement \cite{Bastien1991,Ageorges1998}. 

\begin{equation}
t_r = B_r \, exp \left( \frac{A_r}{T} \right)
\end{equation}

En condition isotherme et pour un temps de reptation donné, les propriétés mécaniques évoluent de façon non-linéaire où le degré de cicatrisation ($D_{h}$) évolue tel que \cite{F.Yang2002} : 
...
Et en observant que les propriétés mécaniques évoluent, pour des temps inférieur au temps de reptation telles que \cite{Wool1983} :
...
D'autres chercheurs ont développés les équations décrivant l'évolution de la cicatrisation pour tenir compte de conditions non-isothermes en discrétisant les phénomènes de contact intime et d'autohésion en fonction du temps  \cite{Bastien1991,F.Yang2002,Ageorges1998}. 

Bastien a pu prévoir la résistance obtenue après un soudage en condition non isotherme, pour des jonctions entre des adhérents en PEI, en se basant sur des données expérimentales provenant d'essais de condition isotherme afin d'estimer les temps de reptation en fonction de la température \cite{Bastien1991}. 
Le modèles de Bastien obtient des temps de reptation du même ordre de grandeur que les temps de reptation évalués expérimentalement (Tab. \ref{tab:temps_de_reptation_Bastien}) en se basant sur la distance moyenne d'interdiffusion plutôt que sur l'évaluation de la longueur des chaînes mineures ayant traversé l'interface. 
...
\begin{table}[h]
	\centering
	\caption{Évaluation des expérimentale des temps de reptation et comparaison avec les résultats des modèles}
	\label{tab:temps_de_reptation_Bastien}
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{@{}lcccc@{}}
			\toprule
			                     & \multicolumn{1}{l}{} &          \multicolumn{3}{c}{Temps de reptation}          \\
			Type d'essai         &          Température &      Résultats &          Critère &  Critère de longueur \\
			                     &                      &   expérimentaux & d'interdiffusion & des chaînes mineures \\
			                     &      [\si{\celsius}] & [\si{\second}] &   [\si{\second}] &       [\si{\second}] \\ \midrule
			Moulage à la presse  &                  230 &        300 880 &          510 000 &            5 400 000 \\
			Essais de fluage     &                  250 &           4800 &             5446 &               99 131 \\
			Mesures en rhéologie &                  270 &          1 - 8 &               81 &                 2437 \\ \bottomrule
		\end{tabular}%
	}
\end{table}

Un modèle subséquent est parvenu à améliorer les prédictions de temps de cicatrisation pour les conditions de variations rapides de température et permet d'évaluer l'état d'une interface soudée avec plus de précision \cite{F.Yang2002}. 
...
D'autres auteurs ont également utilisé des puissances allant jusqu'à \SI[locale=FR]{282}{\kilo\watt\per\square\metre} \cite{Dube2007} avec des temps plus courts. 
Certains auteurs utilisent également de très fortes puissances pulsées de l'ordre de \SI[locale=FR]{600}{\kilo\watt\per\square\metre} avec un temps de repos de quelques secondes entre chaque pulsation afin d'obtenir une température plus uniforme dans la zone soudée et réduire la quantité d'énergie requise  \cite{Arias1996}. 
...
En premier lieu, pour les soudures de laminés avec une matrice en polyétherimide (PEI), Ageorges \cite{Ageorges2000a} a observé des signes de déconsolidation des laminés lorsqu'une faible pression, de l'ordre de \SI[locale=FR]{0.1}{\mega\pascal}, est employée.
...
Les considérations quant à l'emploi d'un nanocomposite dans le cadre d'un procédé entraînant la fonte de la matrice polymère ne sont pas explorés dans la littérature. 
