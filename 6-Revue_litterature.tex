...
Les composites à matrice thermoplastique occupent d'ailleurs un rôle central ce cette transition \cite{CompositesWorld2019_market2020}. 
...
Les différentes méthodes de mise en forme des composites à matrice thermoplastiques sont principalement des variations quant aux méthodes de chauffe et de mise en place des matériaux. 
...
Bastien \cite{Bastien1991} a pu prévoir la résistance obtenue après un soudage en condition non isotherme, pour des jonctions entre du PEI et du PEEK, en se basant sur de données expérimentales provenant d'essais de condition isotherme afin d'estimer les temps de reptation en fonction de la température. 
...
\begin{figure}[]
	...
	\caption{Procédés de soudage (adapté de \cite{Ageorges2001a}, avec permission)}
...
Il s'agit d'une méthode rapide, répétables et pouvant être aisément mise à l'échelle pour produire de plus grandes soudures. 
...
\begin{figure}[]
	...
	\caption{Fenêtre de puissance pour le procédé de soudage (tiré de \cite{Stavrov2005a}, avec permission)}
...
Un nanotube simple parois nécessite une densité énergétique d'approximativement \SI{100}{\mega\pascal} tandis qu'un nanotube multi parois nécessite une densité énergétique de l'ordre de \SI{16}{\kilo\pascal} \cite{Huang2012}. 
...
Pour fin de comparaison, la conductivité électrique de l'acier inoxydable est de \SI{14.5e5}{\siemens\per\metre} soit une valeur 15 fois plus élevé. 
...
Il est possible de disperser les nanotubes de façon à une mauvaise distribution pour obtenir des contacts entre les agrégats \cite{Al-Saleh2009c,Deng2014a}. 
...
\begin{figure}[h]
	...
	\caption{Comparaison entre les effets de la distribution et de la dispersion (tiré de \cite{Al-Saleh2009c}, avec permission)}
...
%La nature d'un \textit{buckypaper} peut être défini ainsi : 
%
%\begin{quote}
%	Feuille mince composée d'un assemblage de nanotubes. 
%\end{quote}

Le \textit{buckypaper} peut être composé de plusieurs sortes de nanoparticules dont entre autre des nanotubes de carbone ou des nanofibres d'argent. 
...
Ces chercheurs ont utilisés autant des \textit{buckypaper} de nanotubes de carbone que des feuilles de nanofibres d'argent combinées à un polymère conducteur. 

Pour les films de nanotubes alignés, un film peut être obtenus en enroulant des nanotubes sur un mandrin ou un cadre depuis un substrat sur lequel une forêt de nanotubes a été produite par déposition chimique en phase vapeur. 
